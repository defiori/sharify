...
// CORRESPONDS TO https://github.com/apache/arrow/blob/1b70f654b7a43a73dd2aa4e3e03b66858d0f9e96/rust/arrow/src/memory.rs

//! Defines memory-related functions, such as allocate/deallocate/reallocate
//! memory regions, cache and allocation alignments.
...
// Memory allocation should align well with usage pattern of cache access and
// block sizes on layers of storage levels from registers to non-volatile
// memory. These alignments are all cache aware alignments incorporated from [cuneiform](https://crates.io/crates/cuneiform) crate. This approach mimicks Intel TBB's
// cache_aligned_allocator which exploits cache locality and minimizes prefetch
// signals resulting in less round trip time between the layers of storage.
...
// Loads data or instructions from memory to the second-level cache. To use the
// streamer, organize the data or instructions in blocks of 128 bytes, aligned
// on 128 bytes.
...
///
/// * new_size, when rounded up to the nearest multiple of [ALIGNMENT], must not
///   overflow (i.e.,
...
/// [`Copy`]. If `T` is not [`Copy`], using both the values in the region
/// beginning at `*src` and the region beginning at `*dst` can [violate memory
/// safety][read-ownership].
